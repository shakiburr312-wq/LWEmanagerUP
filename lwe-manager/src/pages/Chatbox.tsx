...
export const Chatbox: React.FC = () => {
  ...
  const globalPlayers = players.filter(p => p.status !== 'banned');
  const globalOnlineCount = globalPlayers.filter(p => checkIsOnline(p)).length;

  const teamPlayers = players.filter(p => p.lineup === userLineup && p.status !== 'banned');
  const teamOnlineCount = teamPlayers.filter(p => checkIsOnline(p)).length;
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="p-4 md:p-6 bg-[#0a0a0f] border-b border-purple-500/10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 flex-shrink-0">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-1.5">
              ...
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-mono font-bold uppercase">
                <span className="relative flex h-1.5 w-1.5">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500"></span>
                </span>
                Lineup Online: {teamOnlineCount}/{teamPlayers.length}
              </span>
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-purple-500/10 border border-purple-500/20 text-purple-400 text-[10px] font-mono font-bold uppercase">
                <Globe className="w-3 h-3 text-purple-400" />
                Global Online: {globalOnlineCount}/{globalPlayers.length}
              </span>
            </div>
          </div>
        ...
        <div className="flex-1 flex overflow-hidden min-h-0 relative">
          ...
          <div className="hidden lg:flex flex-col w-64 bg-[#0a0a10] border-l border-white/5 h-full flex-shrink-0">
            <div className="p-4 border-b border-white/5 flex items-center justify-between bg-[#0e0e16]/40">
              ...
              <span className="text-[10px] font-mono font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                {activeChannel === 'My Team' ? teamOnlineCount : globalOnlineCount} ON
              </span>
            </div>
