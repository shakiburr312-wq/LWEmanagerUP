        ...
        <header className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-3">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-mono font-bold uppercase">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                </span>
                My Lineup ({user?.lineup || '1st Lineup'}): {players.filter(p => p.lineup === (user?.lineup || '1st Lineup') && p.status !== 'banned' && checkIsOnline(p)).length}/{players.filter(p => p.lineup === (user?.lineup || '1st Lineup') && p.status !== 'banned').length} Online
              </span>
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-purple-500/10 border border-purple-500/20 text-purple-400 text-xs font-mono font-bold uppercase">
                <Globe className="w-3.5 h-3.5" />
                Global Network: {players.filter(p => p.status !== 'banned' && checkIsOnline(p)).length}/{players.filter(p => p.status !== 'banned').length} Online
              </span>
            </div>
          </div>
            ...
            {filteredPlayers.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredPlayers.map((player) => {
                    ...
                    >
                      ...
                      <div className="p-6 border-b border-white/5">
                        <div className="bg-[#050507]/60 border border-white/5 rounded-2xl p-4 grid grid-cols-3 gap-2 mb-3">
                          ...
                          <div className="text-center">
                            <span className="text-[9px] font-mono text-gray-500 uppercase block mb-1">Assists</span>
                            <div className="flex items-center justify-center space-x-1">
                              <Shield className="w-3.5 h-3.5 text-purple-400" />
                              <span className="text-xs font-bold text-white font-mono">{player.assists || 0}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex justify-between items-center px-2 text-[10px] font-mono text-gray-500">
                          ...
                          <span>Lineup: <strong className="text-purple-400 uppercase">{
                            player.role?.toLowerCase() === 'admin' || player.role?.toLowerCase() === 'management'
                              ? 'Neutral'
                              : (() => {
                                  const pLineup = lineups.find(l => l.id === player.lineupId || l.name === player.lineup);
                      ...
                      {player.role?.toLowerCase() === 'admin' || player.role?.toLowerCase() === 'management' ? (
                        <div className="p-6 text-center text-xs text-gray-500 font-mono border-b border-white/5 bg-[#0a0a0f]/40">
                          🛡️ NEUTRAL MANAGEMENT (No Wallet, Salary, or Warnings)
                        </div>
                      ) : (
                  ...
                  {isAdmin && player.role?.toLowerCase() !== 'admin' && player.role?.toLowerCase() !== 'management' && (
