...
export const Finance: React.FC = () => {
  ...
  const [approvalMethod, setApprovalMethod] = useState<'bKash' | 'Nagad' | 'Rocket'>('bKash');
                    ...
                    {salaryRequests.filter(r => r.status === 'pending').map((req) => (
                      <div key={req.id} className="bg-[#050507] border border-white/5 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 font-mono text-xs">
                        <div className="flex-1">
                          ...
                          <div className="text-[10px] text-gray-500 font-sans">Submitted: {new Date(req.date).toLocaleString()}</div>

                          {req.playerMethod && req.playerNumber && (
                            <div className="mt-3 flex flex-wrap items-center gap-2 bg-[#0c0c14] border border-purple-500/15 rounded-xl px-2.5 py-1.5 text-[10px] text-gray-300 max-w-md">
                              <span className="text-purple-400 font-bold uppercase tracking-wider text-[9px]">PAYOUT TO:</span>
                              <span className={`px-2 py-0.5 font-bold rounded text-[9px] uppercase ${
                                req.playerMethod === 'bKash' ? 'bg-[#e2125c]/15 text-[#e2125c] border border-[#e2125c]/20' :
                                req.playerMethod === 'Nagad' ? 'bg-[#f04f23]/15 text-[#f04f23] border border-[#f04f23]/20' :
                                'bg-[#8c2e8c]/15 text-[#8c2e8c] border border-[#8c2e8c]/20'
                              }`}>
                                {req.playerMethod}
                              </span>
                              <span className="text-white font-bold font-mono tracking-wider text-[11px] bg-[#050507] px-2 py-0.5 rounded border border-white/5">
                                {req.playerNumber}
                              </span>
                            </div>
                          )}
                            ...
                            <div className="bg-[#0c0c14] border border-purple-500/20 rounded-xl p-3 space-y-2 max-w-xs">
                              ...
                              <div className="space-y-1">
                                ...
                                <select 
                                  ...
                                  onChange={(e) => setApprovalMethod(e.target.value as 'bKash' | 'Nagad' | 'Rocket')}
                                ...
                                >
                                  ...
                                  <option value="Rocket">Rocket (Dropdown)</option>
                                </select>
                              </div>
                            ...
                            <div className="flex gap-2">
                              <button
                                onClick={() => {
                                  ...
                                  setApprovalMethod((req.playerMethod as any) || 'bKash');
                                }}
