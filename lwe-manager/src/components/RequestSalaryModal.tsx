...
export const RequestSalaryModal: React.FC<RequestSalaryModalProps> = ({ isOpen, onClose, playerProfile }) => {
  ...
  const [playerMethod, setPlayerMethod] = useState<'bKash' | 'Nagad' | 'Rocket'>('bKash');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!playerNumber.trim()) {
      toast.error('Please provide your mobile number for payment!');
      return;
    }
      ...
      <div className="bg-[#0c0c14] max-w-md w-full rounded-3xl border border-purple-500/20 p-6 shadow-2xl relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 font-mono text-xs">
          ...
          <div className="space-y-2">
            <label className="text-[10px] text-gray-400 uppercase tracking-wider block">Payment Method</label>
            <div className="grid grid-cols-3 gap-2">
              {(['bKash', 'Nagad', 'Rocket'] as const).map((method) => {
                const isActive = playerMethod === method;
                const colors = {
                  bKash: isActive 
                    ? 'bg-[#e2125c]/15 border-[#e2125c] text-[#e2125c]' 
                    : 'border-white/5 bg-[#050507]/40 text-gray-400 hover:border-white/10',
                  Nagad: isActive 
                    ? 'bg-[#f04f23]/15 border-[#f04f23] text-[#f04f23]' 
                    : 'border-white/5 bg-[#050507]/40 text-gray-400 hover:border-white/10',
                  Rocket: isActive 
                    ? 'bg-[#8c2e8c]/15 border-[#8c2e8c] text-[#8c2e8c]' 
                    : 'border-white/5 bg-[#050507]/40 text-gray-400 hover:border-white/10',
                };
                return (
                  <button
                    key={method}
                    type="button"
                    onClick={() => setPlayerMethod(method)}
                    className={`py-2 px-3 rounded-xl border text-center font-bold text-xs transition-all cursor-pointer ${colors[method]}`}
                  >
                    {method}
                  </button>
                );
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] text-gray-400 uppercase tracking-wider block">Payment Account Number (মোবাইল নম্বর)</label>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-purple-400">
                #
              </span>
