...
export async function performScoreSync(
  ...
  try {
    ...
    for (const player of activePlayers) {
      ...
      const totalAssists = playerLogs.reduce((sum, log) => sum + (Number(log.assists) || 0), 0);
      const totalHealing = playerLogs.reduce((sum, log) => sum + (Number(log.healing) || 0), 0);
      ...
      const hasDiff = 
        ...
        (player.assists || 0) !== totalAssists ||
        (player.healing || 0) !== totalHealing ||
...
export async function checkAndTriggerDailySync(
  ...
  // Fast check in local storage - prevent any further checks or queries if already synced today
  ...
  try {
    ...
    // If today is different from last sync date, OR we have out-of-sync stats due to formula, trigger sync!
    if (lastDbSyncDate !== todayStr || needsFormulaSync) {
      if (isAdmin && players.length > 0 && performanceLogs.length > 0) {
        ...
        // If they are not admin, they cannot sync. We still cache it to prevent spamming Firestore reads on every render
        localStorage.setItem(LOCAL_STORAGE_SYNC_KEY, todayStr);
      }
    } else {
    ...
    // Cache anyway on failure to prevent aggressive loop on error
    localStorage.setItem(LOCAL_STORAGE_SYNC_KEY, todayStr);
  }
